...
def main() -> None:
    ...
    if not active_session_id:
        ...
        current_round_no = int(session["current_round"])
        round_row = db.get_round(int(session["id"]), current_round_no)

        if not round_row:
            prev_round_no = current_round_no - 1
            prev_round = db.get_round(int(session["id"]), prev_round_no)
            if prev_round:
                prev_questions = db.get_round_questions(int(prev_round["id"]))
                prev_answers = db.get_round_answers(int(prev_round["id"]))
                failed_qcodes = [
                ...
                if failed_qcodes and session["retry_mode"] == "manual":
                    st.warning(
                        "Manual retry mode: previous round completed. Start the next failed-only round when ready."
                    )
                    if st.button("Start failed-only retry round", type="primary"):
                        new_round = db.create_round(int(session["id"]), current_round_no, failed_qcodes)
                        st.success(
                            f"Started round {current_round_no} with {len(failed_qcodes)} failed questions."
                        )
                        round_row = {"id": new_round, "round_number": current_round_no}
                elif not failed_qcodes:
                    st.success("All questions answered correctly. Session complete.")
            if not round_row:
                return
        ...
        metric_cols[1].metric("Open", stats["open"])
        metric_cols[2].metric("Filled", stats["filled"])
        metric_cols[3].metric("Correct", stats["correct"])
        metric_cols[4].metric("Wrong", stats["wrong"])

        st.markdown(
            f"Session #{session['id']} - **{session['name']}** | "
            f"Round {round_row['round_number']} | "
            f"Mode: {session['check_mode']} / {session['retry_mode']}"
        )
        ...
        for i, q in enumerate(round_questions):
            icon = get_status_icon(answers.get(q["qcode"], {}))
            if nav_cols[i % 6].button(f"{icon} {q['qcode']}", key=f"nav_{round_id}_{q['qcode']}"):
                st.session_state["question_idx"] = i
        ...
        with col_actions[0]:
            if st.button("✓ Save answer", type="primary", use_container_width=True):
                if session["check_mode"] == "immediate":
                    ...
                    st.success("✓ Saved (will be checked on submit)")
                    st.rerun()
